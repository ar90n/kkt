...
def create_bootstrap_cell(pkg_name: str, pkg_encoded: str):
    bootstrap_code = create_bootstrap_code(pkg_name=pkg_name, pkg_encoded=pkg_encoded)
    return {
        "cell_type": "code",
...
def create_notebook_kernel(notebook_body: str, pkg_name: str, pkg_encoded: str):
    bootstrap_code = create_bootstrap_code(pkg_name=pkg_name, pkg_encoded=pkg_encoded)

    notebook_obj = erace_all_outputs(json.loads(notebook_body))

    bootstrap_cell = create_bootstrap_cell(pkg_name=pkg_name, pkg_encoded=pkg_encoded)
